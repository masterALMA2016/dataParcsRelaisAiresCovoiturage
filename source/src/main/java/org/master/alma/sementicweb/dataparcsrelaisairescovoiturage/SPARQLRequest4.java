 ...
 */
public class SPARQLRequest4 extends SPARQLRequest {
    ...
    void execute() {
        ...
        String queryString =
                ...
                "SELECT ?nameLieuMobilite ?carCapacity "+
                "WHERE { " +
                "      ?mobilite rdf:type pdll:mobilite . " +
                "      ?mobilite foaf:name ?nameLieuMobilite . " +
                "      OPTIONAL { ?same pdll:carCapacity ?carCapacity . ?same owl:sameAs ?mobilite }"+
                " } ";

        Query query = QueryFactory.create(queryString);
        QueryExecution qe = QueryExecutionFactory.create(query, m);
        ResultSet results = qe.execSelect();
        System.out.println("Liste des équipements de mobilité avec leur nombre de places s'il s'agit d'un parking");
